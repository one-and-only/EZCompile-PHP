...
if (!(docker ps -q -f name=mysql57)) {
    if ((docker ps -aq -f status=exited -f name=mysql57) -or (docker ps -aq -f status=created -f name=mysql57)) {
        # cleanup
        docker rm mysql57
    }
    elseif (docker ps -aq -f status=running -f name=mysql57) {
        $mysqlstatus = "already-running"
    }
    # run your container
    if (docker run -d -p 3306:3306 -p 33060:33060 --volume ${MYSQLFOLDERLOCATION}:/var/lib/mysql --name mysql57 frostedflakez/php-mysql-webserver:mysql-latest-5.7) {
        $mysqlstatus = "started"
    }
    else {
        $mysqlstatus = "error"
