...
if (!(docker ps -q -f name=php74)) {
    if ((docker ps -aq -f status=exited -f name=php74) -or (docker ps -aq -f status=created -f name=php74)) {
        # cleanup
        docker rm php74
    }
    elseif (docker ps -aq -f status=running -f name=php74) {
        $mysqlstatus = "already-running"
    }
    # run your container
    if (docker run -d -p 80:80 -p 443:443 --volume ${APACHEFOLDERLOCATION}:/var/www/html/ --name php74 frostedflakez/php-mysql-webserver:php-latest-7.4) {
        $phpstatus = "started"
    }
    else {
        $phpstatus = "error"
