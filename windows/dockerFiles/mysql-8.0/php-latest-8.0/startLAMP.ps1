...
if (!(docker ps -q -f name=php80)) {
    if ((docker ps -aq -f status=exited -f name=php80) -or (docker ps -aq -f status=created -f name=php80)) {
        # cleanup
        docker rm php80
    }
    elseif (docker ps -aq -f status=running -f name=php80) {
        $mysqlstatus = "already-running"
    }
    # run your container
    if (docker run -d -p 80:80 -p 443:443 --volume ${APACHEFOLDERLOCATION}:/var/www/html/ --name php80 frostedflakez/php-mysql-webserver:php-latest-8.0) {
        $phpstatus = "started"
    }
    else {
        $phpstatus = "error"
