...
if (!(docker ps -q -f name=php73)) {
    if ((docker ps -aq -f status=exited -f name=php73) -or (docker ps -aq -f status=created -f name=php73)) {
        # cleanup
        docker rm php73
    }
    elseif (docker ps -aq -f status=running -f name=php73) {
        $mysqlstatus = "already-running"
    }
    # run your container
    if (docker run -d -p 80:80 -p 443:443 --volume ${APACHEFOLDERLOCATION}:/var/www/html/ --name php73 frostedflakez/php-mysql-webserver:php-latest-7.3) {
        $phpstatus = "started"
    }
    else {
        $phpstatus = "error"
...
if (!(docker ps -q -f name=mysql80)) {
    if ((docker ps -aq -f status=exited -f name=mysql80) -or (docker ps -aq -f status=created -f name=mysql80)) {
        # cleanup
        docker rm mysql80
    }
    elseif (docker ps -aq -f status=running -f name=mysql80) {
        $mysqlstatus = "already-running"
    }
    # run your container
    if (docker run -d -p 3306:3306 -p 33060:33060 --volume ${MYSQLFOLDERLOCATION}:/var/lib/mysql --name mysql80 frostedflakez/php-mysql-webserver:mysql-latest-8.0) {
        $mysqlstatus = "started"
    }
    else {
        $mysqlstatus = "error"
